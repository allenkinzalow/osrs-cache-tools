...
import io.guthix.osrs.cache.export.gltf.*
...
fun gltfExport(id: Int, model: Model) {
    ...
    GltfWriter().write(gltf,  FileOutputStream(File("out/gltf/$id.gltf")))
    FileOutputStream(File("out/gltf/$id.bin")).write(buffer.array())
}
        ...
        if (textureId == -1) { // Triangle has color
            ...
            if(colorIndices.isEmpty()) { // new color
                val alpha = if(model.triangleAlphas != null) {
                    ((model.triangleAlphas!![triangleId] and 0xFF.toByte()) / 255).toFloat()
                } else 1f
                val material = createColorMaterial(rs2hsbToColor(colorBitPack), alpha)
